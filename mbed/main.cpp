...
int main()
{
    ...
    // On/Off, Mode, Send
    DigitalOut controls[] = { p13, p14, p15 };

    // A, B, C, D, E
    DigitalOut selections[] = { p16, p17, p18, p19, p21 };
		
		// Used for reading the clicker's voltage regulator output for determining if clicker is on/off
		AnalogIn pwr_status(p20);
    ...
    // Initialize power toggle pin so clicker stays off at mbed startup
    controls[0] = 1;
